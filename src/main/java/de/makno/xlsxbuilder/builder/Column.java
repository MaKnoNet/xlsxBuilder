...
public final class Column<T> {
    ...
    /** Paket-intern: Typ der Spalte setzen (vom {@code XlsxBuilder.ofType(...)} genutzt). */
    void setType(ColumnType type) {
        this.type = Objects.requireNonNull(type, "type");
    }

    /** Paket-intern: Format-Code der Spalte setzen (vom {@code XlsxBuilder.formatForType(...)} genutzt). */
    void setFormat(String format) {
        this.format = format;
    }

    /** Paket-intern: spalten-spezifischen Null-Platzhalter setzen (vom {@code XlsxBuilder.nullText(...)}). */
    void setNullText(String nullText) {
        this.nullText = nullText;
    }

    /** Paket-intern: optionalen Wert-Konverter setzen (vom {@code XlsxBuilder.convertToColumnType(...)}). */
